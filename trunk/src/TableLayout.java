...
//      if (rowWeights[i] == 0.0) fixedHeight += Math.max(rowSizeHints[i],rowSizeMin[i]);
      if (!rowExpandFlags[i]) fixedHeight += Math.max(rowSizeHints[i],rowSizeMin[i]);
...
if (debug) { System.err.print(indent()+"row size min     : ");for (int i = 0; i < rows;    i++) System.err.print(" "+rowSizeMin[i]);System.err.println(); }
